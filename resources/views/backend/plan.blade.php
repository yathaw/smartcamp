<x-template>
	...
	<div class="pagetitle">
	    <h1> {{ __("Pricing Plan")}}</h1>
	    <nav>
	        <ol class="breadcrumb">
	            <li class="breadcrumb-item"><a href="">{{ __("Dashboard")}}</a></li>
	            <li class="breadcrumb-item active">{{ __("All Pricing Plans")}}</li>
	        </ol>
	    </nav>
	</div>
	<!-- End Page Title -->
	            ...
	            <form class="row g-3">
		            ...
		            <div class="modal-body">
					    ...
					    <div class="col-12 my-3">
                            ...
                            <span class="err_amount error d-block text-danger"></span>

					    </div>
					    <div class="col-12 mt-3">
					        <label for="inputDuration" class="form-label">{{ __("Duration")}}</label>
					        <input type="text" class="form-control" id="inputDuration" name="duration">

                            <span class="err_duration error d-block text-danger"></span>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $('.datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.getlistPlans') }}",
                columns: [
            ...
            $("#showModal").on('submit','#addForm',function(e){
                ...
                $.ajax({
                    ...
                    success: (data) => { 
                        ...
                        $('.err_amount').remove();
                        $('#inputamount').removeClass('border border-danger');

                        $('.err_duration').remove();
                        $('#inputduration').removeClass('border border-danger');
                    ...
                    error: function(error){
                        ...
                        $.each(err, function( key, value ) {
                            ...
                            if (key == "amount") 
                            {
                                $('.err_amount').html(err[key]);
                                $('#inputamount').addClass('border border-danger');
                            }

                            if (key == "duration") 
                            {
                                $('.err_duration').html(err[key]);
                                $('#inputduration').addClass('border border-danger');
                            }
            ...
            $("#showModal").on('submit','#editForm',function(e){
                ...
                var url="{{route('master.plan.update',':id')}}";
            ...
            $('tbody').on('click', '.deleteBtn', function () {
                ...
                var url="{{route('master.plan.destroy',':id')}}";
                url=url.replace(':id',id);
