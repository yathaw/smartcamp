	...
	<div class="pagetitle">
	    ...
	    <nav>
	        <ol class="breadcrumb">
	            ...
	            <li class="breadcrumb-item active">{{ __("All Lessons")}}</li>
	        </ol>
	    </nav>
	</div>
			                                    ...
			                                    <div class="accordion-body row g-5 p-5">
			                                        ...
			                                        @foreach($grade->lessons as $lesson)
			                                            @php
			                                                $cover = $lesson->photo;
			                                                $file = $lesson->file;
			                                                $subject = $lesson->curriculum->subject->name;
			                                                $type = $lesson->curriculum->type;

			                                                $lessons_id = $hashids->encode($lesson->id);
			                                            @endphp
			                                            <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12" >
			                                            	<div class="card shadow-none border p-0">
															  	<img src="{{ asset($cover) }}" class="card-img-top" alt="...">
															  	
															  	<div class="card-body">
															  		<h5 class="card-title">{{ $subject }}</h5>

															  		<div class="d-grid gap-2">
																	  	<a href="{{ route('master.lesson.show', $lessons_id) }}" class="btn btn-primary">
																	  		<i class="bi bi-play-fill me-2"></i> View Video 
																	  	</a>
																	</div>
																	@if(!in_array($authRole,["Guardian","Student"]))
																	<div class="d-grid gap-2 d-md-flex justify-content-md-end mt-3">
																	  	<a href="{{ route('master.lesson.edit', $lessons_id) }}" class="btn btn-outline-warning me-md-2 btn-sm" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ __('Edit') }}"  >
																	  		<i class="bi bi-gear-fill"></i> 
																	  	</a>
																	  	<a href="javascript:void(0)" class="btn btn-outline-danger btn-sm deleteBtn" data-id="{{ $lesson->id }}" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ __('Remove') }}" >
	        ...
	        <div class="col-lg-12">
	            <div class="card">
	                <div class="card-body pt-4 d-flex flex-column align-items-center">
	                    ...
	                    <h2> {{ __("No Lesson Data Found") }} </h2>
	                    <p> {{ __("There have been no Lesson in this section yet. Please add some Lesson first.") }} </p>

	                    <div class="d-grid gap-2 col-6 mx-auto my-5">
						  	<a href="{{ route('master.lesson.create') }}" class="btn btn-primary"> <i class="bi bi-plus-lg"></i> {{ __("Add Lesson")}} </a>
						</div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.accordion-item').on('click', '.deleteBtn', function () {
                ...
                var url="{{route('master.lesson.destroy',':id')}}";
