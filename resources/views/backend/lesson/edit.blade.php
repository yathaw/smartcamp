	...
	<div class="pagetitle">
	    ...
	    <nav>
	        <ol class="breadcrumb">
	            ...
	            <li class="breadcrumb-item active">{{ __("Edit Existing Lesson")}}</li>
	        </ol>
	    </nav>
	</div>
	        ...
	        <div class="col-lg-12">
	        	<div class="card">
	            	<div class="card-header row align-items-center">
	            		<div class="col-12">
	                    	{{ __("Edit Existing lesson") }}
	            		</div>
	            	</div>
	                <form class="new-added-form" action="{{route('master.lesson.update',$lesson->id)}}" enctype="multipart/form-data" method="POST">
		            @csrf
                    @method('PUT')

                        <input type="hidden" name="oldphoto" value="{{ $lesson->photo }}">
                        <input type="hidden" name="oldfile" value="{{ $lesson->file }}">
		                    ...
		                    <div class="row">
		                    	<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
		                    		<div class="form-group mb-lg-5 mb-3" id="gradeDiv">
			                            ...
			                            <select class="select2 inputGrades @if ($errors->has('grade')) is-invalid @endif @if($errors->any() && !$errors->has('grade')) is-valid @endif" name="grade" id="inputGrade">
			                                ...
			                                @foreach($grades as $grade)
			                                    <option value="{{ $grade->id }}" @if($grade->id == $lesson->curriculum->grade_id)selected @endif>
			                                        {{ $grade->name }} 
			                                    </option>
			                                @endforeach
			                            </select>
			                        ...
			                        <div class="form-group mb-lg-5 mb-3">
			                            <label for="inputPDF" class="d-block"> {{ __("File") }} *</label>

                                        <ul class="nav nav-tabs mb-3 nav-justified" id="myTab" role="tablist">
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link active" id="oldfile-tab" data-bs-toggle="tab" data-bs-target="#oldfile" type="button" role="tab" aria-controls="oldfile" aria-selected="true">{{ __("Old File") }}</button>
                                            </li>
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link" id="newfile-tab" data-bs-toggle="tab" data-bs-target="#newfile" type="button" role="tab" aria-controls="newfile" aria-selected="false">{{ __("New File") }}</button>
                                            </li>
                                        </ul>
                                        <div class="tab-content" id="myTabContent">
                                            <div class="tab-pane fade show active" id="oldfile" role="tabpanel" aria-labelledby="oldfile-tab">
                                                <video id="player" playsinline controls data-poster="{{ asset($lesson->photo) }}" style="--plyr-color-main: #192060;">
                                                    <source src="{{ asset($lesson->file) }}" type="video/mp4" />
                                                    <source src="{{ asset($lesson->file) }}" type="video/webm" />
		                    	...
		                    	<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 form-group mb-3 imgUp">
                                    ...
                                    <div class="imagePreview" style="background-image: url({{ asset($lesson->photo) }});"></div>
...
@section('script_content')
	<script type="text/javascript">
        ...
        var gid = "{{ $lesson->curriculum->grade_id }}";
        var subjectid = "{{ $lesson->curriculum->subject_id }}";

        var url = "{{ asset($lesson->file) }}";
    </script>
