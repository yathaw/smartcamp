	...
	<div class="pagetitle">
	    ...
	    <nav>
	        <ol class="breadcrumb">
	            ...
	            <li class="breadcrumb-item active">{{ __("All Cities")}}</li>
	        </ol>
	    </nav>
	</div>
	<!-- End Page Title -->
	                        ...
	                        <tbody>
	                            ...
	                            @foreach($cities as $city)
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-outline-warning me-2 editBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __("Edit")}}" data-id="{{ $city->id }}" data-name="{{ $city->name }}" data-stateid="{{ $city->state_id }}">
                                                <i class="bi bi-gear-fill"></i> 
                                            </button>
                                            <button type="button" class="btn btn-outline-danger me-2 deleteBtn" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __("Remove")}}" data-id="{{ $city->id }}">
	            ...
	            <form class="row g-3">
		            ...
		            <div class="modal-body">
					    ...
					    <div class="col-12 mt-3">
					        <label for="inputName" class="form-label">{{ __("State")}}</label>
					        <select class="form-control select2" name="stateid" id="inputStateid">
                            </select>

                            <span class="err_stateid error d-block text-danger"></span>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
	        ...
	        function getStates(){
	        	$.ajax({
	                ...
	                success: function(response){
	                	...
	                	$('#inputStateid').select2({
				            width: '100%',
				            placeholder: 'Select a State',
				            dropdownParent: $("#showModal")
				            
				        });
	                    $('#inputStateid').html(html);
            ...
            $("#showModal").on('submit','#addForm',function(e){
                ...
                $.ajax({
                    ...
                    success: (data) => { 
                        ...
                        $('.err_stateid').remove();
                        $('#inputStateid').removeClass('border border-danger');
                    ...
                    error: function(error){
                        ...
                        $.each(err, function( key, value ) {
                            ...
                            if (key == "stateid") 
                            {
                                $('.err_stateid').html(err[key]);
                                $('#inputStateid').addClass('border border-danger');
                            }
            ...
            $('tbody').on('click', '.editBtn', function (){
                ...
                $('#inputId').val(id);
                $('#inputName').val(name);
            	$('#inputStateid').val(stateid).trigger('change');
            ...
            $("#showModal").on('submit','#editForm',function(e){
                ...
                var url="{{route('master.city.update',':id')}}";
            ...
            $('tbody').on('click', '.deleteBtn', function () {
                ...
                var url="{{route('master.city.destroy',':id')}}";
                url=url.replace(':id',id);
