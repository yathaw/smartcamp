		        ...
		        <div class="col-lg-12">
		        	<div class="card">
		                ...
		                <div class="card-body pt-3">
		                	...
		                	<form class="new-added-form" action="{{ route('master.installment') }}" method="POST" enctype="multipart/form-data">
			                    @csrf
			                    ...
			                    <div class="row mb-3">
			                    	...
			                    	<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12 ">
				                    	...
				                    	<div class="col-12  form-group mb-3">
				                            ...
				                            <select class="select2" name="student" id="inputStudent" disabled="">
			                   				</select>
...
@section('script_content')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
        	...
        	var selected_studentid = "{{ $studentid  ?? '' }}";
        	...
        	if(selected_periodid){
        		...
        		pickStudent(selected_batchid);
        		pickPackage(selected_studentid, selected_sectionid);
        		pickAmount(selected_packageid);
        	}
		    ...
		    $('#inputBatch').change(function (e) {
		        ...
		        pickStudent(batch_id);

		    });

		    function pickStudent(batch_id){
		        $('#inputStudent').prop('disabled',false);

		        $.ajax({
		            url: "/getStudentsegments_bybatchid",
		            type:'POST',
		            data: { id:batch_id }
		        }).done(function(data){
		            var students = data;
		            var studenthtml ='<option></option>';
		                console.log(selected_studentid);

		            $.each(students,function (i,v) {		                
		                studenthtml +=`<option value="${v.id}"`;
		                if(selected_studentid == v.id){
		                	studenthtml += `selected`;
		                }
		                studenthtml += `>${v.user.name}</option>`;
		            });

		            $('#inputStudent').html(studenthtml);

		        });
		        
		    }

		    $('#inputStudent').change(function (e) {
		        var student_id = $(this).val();
		        var section_id = $('#inputSection').val();
		        pickPackage(student_id, section_id);

		    });

		   	function pickPackage(student_id, section_id){
		   		console.log(section_id);
		        $('#inputPackage').prop('disabled',false);

		        $.ajax({
		            url: "/getPackage_bystudentid",
		            type:'POST',
		            data: { 
		            	id:student_id,
		            ...
		            $.each(packages,function (i,v) {
		                packagehtml +=`<option value="${v.id}"`

		                $.each(studentpayments,function (a,b) {
		                	if(v.id == b){
		                		packagehtml += 'disabled'
		                	}
		                });

		                if(selected_packageid == v.id){
		                	packagehtml += `selected`;
		                }

		                packagehtml+=`>${v.installment}</option>`;
		            });
		    ...
		    $('#inputPackage').change(function (e) {
		   	...
		   	function pickAmount(package_id){
		   		$.ajax({
