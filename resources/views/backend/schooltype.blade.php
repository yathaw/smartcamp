<x-template>
	...
	<div class="pagetitle">
	    <h1> {{ __("School Type")}}</h1>
	    <nav>
	        <ol class="breadcrumb">
	            <li class="breadcrumb-item"><a href="">{{ __("Dashboard")}}</a></li>
	            <li class="breadcrumb-item active">{{ __("All School Types")}}</li>
	        </ol>
	    </nav>
	</div>
	<!-- End Page Title -->
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var table = $('.datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.getlistSchooltypes') }}",
                columns: [
            ...
            $("#showModal").on('submit','#addForm',function(e){
                ...
                $.ajax({
                    type:'POST',
                    url: "{{ route('master.schooltype.store')}}",
                    data: formData,
                    cache:false,
                    contentType: false,
                    processData: false,
            ...
            $("#showModal").on('submit','#editForm',function(e){
                ...
                var url="{{route('master.schooltype.update',':id')}}";
            ...
            $('tbody').on('click', '.deleteBtn', function () {
                ...
                var url="{{route('master.schooltype.destroy',':id')}}";
                url=url.replace(':id',id);
