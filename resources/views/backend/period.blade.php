	...
	<div class="pagetitle">
	    ...
	    <nav>
	        <ol class="breadcrumb">
	            ...
	            <li class="breadcrumb-item active">{{ __("All Periods")}}</li>
	        </ol>
	    </nav>
	</div>
	<!-- End Page Title -->
            ...
            <div class="container">
	            <form class="row g-3">
		            ...
		            <div class="modal-body">
                        ...
                        <div class="col-12 mb-3">
                            <label for="inputSyear" class="form-label">{{ __("Start Year")}}</label>
                            <input type="text" name="startyear" id="inputSyear" class="form-control">

                            <span class="err_startyear error d-block text-danger"></span>

                        </div>

                        <div class="col-12 mb-3">
                            <label for="inputEyear" class="form-label">{{ __("End Year")}}</label>
                            <input type="text" name="endyear" id="inputEyear" class="form-control">

                            <span class="err_endyear error d-block text-danger"></span>
...
@section('script_content')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            const startyear_datepicker = new Datepicker(startyear, {
                autohide: true,
                pickLevel: 2,
                'format': 'yyyy',
                minDate: new Date(),
            ...
            const endyear_datepicker = new Datepicker(endyear, {
                autohide: true,
                pickLevel: 2,
                'format': 'yyyy',
                minDate: new Date(),
            ...
            var table = $('.datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.getlistPeriods') }}",
                columns: [
                ...
                $.ajax({
                    ...
                    error: function(error){
                        ...
                        $.each(err, function( key, value ) {
                            ...
                            if (key == "startyear") 
                            {
                                $('.err_startyear').html(err[key]);
                                $('#inputSyear').addClass('border border-danger');
                            }

                            if (key == "endyear") 
                            {
                                $('.err_endyear').html(err[key]);
                                $('#inputEyear').addClass('border border-danger');
                            }
            ...
            $('tbody').on('click', '.editBtn', function (){
                ...
                $('#inputId').val(id);
                $('#inputName').val(name);
                $('#inputSyear').val(startyear);
                $('#inputEyear').val(endyear);
            ...
            $("#showModal").on('submit','#editForm',function(e){
                ...
                var url="{{route('master.period.update',':id')}}";
            ...
            $('tbody').on('click', '.deleteBtn', function () {
                ...
                var url="{{route('master.period.destroy',':id')}}";
                url=url.replace(':id',id);
